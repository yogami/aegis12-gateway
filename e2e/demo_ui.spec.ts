...
// Use the PHALA_IP_ADDRESS if available, otherwise default to localhost.
const getBaseUrl = () => {
    const ip = process.env.PHALA_IP_ADDRESS;
    return ip ? `http://${ip}:8000` : 'http://localhost:8000';
};
...
test.describe('Aegis-12 Demo Console UI Verification', () => {
    ...
    test('should execute a valid trade and verify SSE log stream', async ({ page }) => {
        ...
        await expect(page.locator('.status-badge')).toContainText('TEE SECURE ENCLAVE ACTIVE');

...
import phalaEntrypoint, { pep } from '../../src/application/PhalaEntrypoint';
...
describe('AegisPEP & Phala Entrypoint (E2E)', () => {
    ...
    beforeAll(() => {
        ...
        pep.provisionTestKey('tenant-1', signer.getAddress());
    });
    ...
    it('should approve a legitimate trading request within policy bounds', async () => {
        const policyConfig = { policyId: "pol-1", tenantId: 'tenant-1', version: "1.0", chainId: 1399811149, crossChainTarget: "solana", nonce: `nonce-${Date.now()}-1`, expiresAt: Math.floor(Date.now() / 1000) + 1000, maxAnomalyScore: 100, financialLimitsString: '{"T4": 100000}' };
    ...
    it('should enact a TERMINAL REFUSAL when T4 maximum value limit is exceeded', async () => {
        const policyConfig = { policyId: "pol-2", tenantId: 'tenant-1', version: "1.0", chainId: 1399811149, crossChainTarget: "solana", nonce: `nonce-${Date.now()}-2`, expiresAt: Math.floor(Date.now() / 1000) + 1000, maxAnomalyScore: 100, financialLimitsString: '{"T4": 100000}' };
        const signature = await signer.signEIP712(domain, types, policyConfig);
        ...
        expect(res.error).toContain('exceeds mathematically signed Tier limit');
    });

    it('should enact a TERMINAL REFUSAL when Anomaly score is too high', async () => {
        const policyConfig = { policyId: "pol-3", tenantId: 'tenant-1', version: "1.0", chainId: 1399811149, crossChainTarget: "solana", nonce: `nonce-${Date.now()}-3`, expiresAt: Math.floor(Date.now() / 1000) + 1000, maxAnomalyScore: 80, financialLimitsString: '{"T4": 100000}' };
        const signature = await signer.signEIP712(domain, types, policyConfig);
        ...
        expect(res.status).toBe('denied');
        expect(res.error).toContain('Anomaly score exceeds Dynamic TEE threshold');
    });
